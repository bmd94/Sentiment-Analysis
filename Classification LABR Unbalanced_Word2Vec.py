...
df = pd.read_table("LABR.tsv",header=0, sep='\t')


X=df.review[df.rate==5].iloc[:11756]
X=X.append(df.review[df.rate==1])
y=df.rate[df.rate==5].iloc[:11756]
...
results = open("Resultat_LABR_Unbalanced_Word2Vec.txt",'w')
